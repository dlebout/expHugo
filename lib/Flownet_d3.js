...
function startTransitionWithColor(link, decalage, color){
  console.log(color);
  function motionDashColor(start, offset, color) {
    link.attr('stroke-dashoffset', start)
      ...
      .transition()
        ...
        .attr('stroke', color)
        .on('end', function() {
          motionDashColor(start - decalage, offset - decalage, color);
        });
  }
  motionDashColor(0, -decalage, color)
}
...
function createColorGate(data, color){
  console.log(color);
  var lineFunction = d3.line()
                    .x(function(d) { return d.x; })
                    .y(function(d) { return d.y; })
                    .curve(d3.curveBasis)
  console.log(data);
  var l = d3.select('svg').append('g').selectAll('path')
    .data(data)
    .enter()
    .append('path')
    .attr('id',function(d){ return "l"+d.id})
  ...
  d3.select('svg').append('g').selectAll('path')
    ...
    .each(function(d){
      console.log(color);
      startTransitionWithColor(d3.select(this), d.decalage, color )
    })
}
...
flownet_d3_graph.prototype.addColorGate = function(link, d, color){
  console.log(link, d, color);
  var copy1 = JSON.parse(JSON.stringify(d))
  var copy2 = JSON.parse(JSON.stringify(d))

  var cut = link.getPointAtLength(d.decalage*10)

  d3.select("#l"+d.id).remove()
  d3.select("#l"+d.id).remove()

  copy1.points[1] = {x:cut.x, y:cut.y}
  copy2.points[0] = {x:cut.x, y:cut.y}
  copy2.sizeParticule = 2
  createLink([copy1,copy2])
  //createColorGate([copy3], color)
}
